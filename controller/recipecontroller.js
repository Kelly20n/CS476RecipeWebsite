    ...
    if(GeneralFunctions.verifyUser(ctx) === true)
    {
        ...
        return User.findOne({username: payload.userEmail}).then(async function(loggedUser) {
            if(ctx.file.contentType === 'image/jpeg' || ctx.file.contentType === 'image/png') {
                ...
                if(key != true)
                {
                    //if no recipe exists with the same title, create a new recipe sending the ctx to the "factory" giving it the information of which category it should go in (breakfast, lunch, supper);
                    RecipeFunctions.createRecipe(ctx, loggedUser);
                }
                else return
            }
            else {
